function [var, A, U,U2, B1,B2, var1] = unfold_var_4x4(p, lambda)
% p should be between 0 and 0.2, lambda should be between 0 and 1
% Function to compute specific matrices for given p and lambda
% Returns var, A, U, U2, B1, B2 and var1

% Define matrix A based on parameter p
A = [1-p, p, 0, 0; p, 1-2*p, p, 0; 0, p, 1-2*p, p; 0, 0, p, 1-p];

% Inverse of matrix A
%A_inv = inv(A);

% Define vector R2 and matrix R
R = [-1, 1, 0, 0; 1, -2, 1, 0; 0, 1, -2, 1;0, 0, 1, -1];
R2 = [-1, 3, -3, 1];
...
%Or choose covariances: i different to j, both between 1 and 4
%var = var1(i,j);

%Grafica lambda/var(/covar)
%f=@(lambda) unfold_var_4x4(0,lambda);
