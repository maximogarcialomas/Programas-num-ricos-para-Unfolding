function [bv,x] = unfoldnxnTU_tradeoff_bien(p, lambda, M, x0,n)
...
x = (A' * A + lambda * (R'*R)) \ (A'*A + lambda*(R'*R)*M)*x0';
B=(A' * A + lambda * (R'*R)) \ (A'*A + lambda*(R'*R)*M)-eye(n);
U=(A' * A + lambda * (R'*R)) \ A';
var1 = U' * U;
x=x';
B=B.^2;
...
%f=@(lambda) unfoldnxnTU_tradeoff_bien(p, lambda, M, x0,n);
